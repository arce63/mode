...
exports.run = async (client, message, args) => {
  ...
  if (
    ![client.config.banhammer].some(role =>
  ...
  if (!kisi)
    return message.channel
      .send(
        new MessageEmbed()
          .setDescription(`${message.author} bir ID belirtmelisin.`)
          .setColor("0x800d0d")
          .setAuthor(
            message.member.displayName,
            message.author.avatarURL({ dynamic: true })
  ...
  message.channel
    .send(
      new MessageEmbed()
        .setDescription(
          `${message.author} tarafından ${kisi} adlı kullanıcının sunucu yasağı kaldırıldı.`
        )
        .setColor("0x348f36")
        .setAuthor(
          message.member.displayName,
