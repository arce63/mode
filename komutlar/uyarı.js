...
exports.run = async (client, message, args) => {
  if (
    !message.member.roles.cache.has(client.config.commander) &&
  ...
  if (!kullanıcı)
    return message.channel.send(
      new Discord.MessageEmbed()
        .addField(client.config.sunucuadı, `Bir kullanıcı etiketlemelisin!`)
        .setColor("2e0101")
        .setFooter(message.author.tag, message.author.avatarURL)
        .setTimestamp()
    );
  ...
  member.roles.add(client.config.uyarı); "852714380872908807" //rol

  const kanal = message.guild.channels.cache.find(
    c => c.id == client.config.uyarılog
  );
